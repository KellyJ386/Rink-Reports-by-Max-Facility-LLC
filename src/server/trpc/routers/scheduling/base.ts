...
import { checkScheduleLock, checkDoubleBooking } from "./validation";

/**
 * Parse a JSON value into validated AvailabilityBlock[].
 * Exported so templates.ts and other sub-routers can reuse it.
 */
export function asBlocks(value: unknown): AvailabilityBlock[] {
 ...
 * least 'manager' for their facility. Exported so other sub-routers
 * (templates, swaps, timeOff) can reuse it.
 */
export async function requireManager(ctx: {
  user: { id: string };
  ...
  if (!data || (data.role !== "admin" && data.role !== "manager" && data.role !== "super_admin")) {
    throw new TRPCError({
      code: "FORBIDDEN",
...
// -------------------------------------------------------------------
// Individual procedure exports — composed in index.ts
// -------------------------------------------------------------------

export const listRoster = protectedProcedure.query(async ({ ctx }) => {
  const { data, error } = await ctx.supabase
    .from("user_profiles")
...
export const listPositions = protectedProcedure.query(async ({ ctx }) => {
  const { data, error } = await ctx.supabase
...
export const listCertifications = protectedProcedure.query(async ({ ctx }) => {
  const { data, error } = await ctx.supabase
...
export const saveMyAvailability = protectedProcedure
...
export const listFacilityAvailability = protectedProcedure
  .input(z.object({ week_start: z.string().regex(/^\d{4}-\d{2}-\d{2}$/) }))
  .query(async ({ ctx, input }) => {
    const { data, error } = await ctx.supabase
      .from("scheduling_availability")
...
export const getScheduleForWeek = protectedProcedure
  .input(z.object({ week_start: z.string().regex(/^\d{4}-\d{2}-\d{2}$/) }))
  .query(async ({ ctx, input }): Promise<{
    ...
    const rawSchedule = schedule as unknown as Record<string, unknown>;
    const typedSchedule: Schedule = {
      ...
      is_locked: rawSchedule.is_locked === true,
    };
    ...
    return {
      ...
      shifts: (shifts ?? []).map((s) => {
        ...
        return {
          ...
          end_at: s.end_at,
          notes: s.notes,
          is_mod: raw.is_mod === true,
          is_open: raw.is_open === true,
          status: (raw.status as "confirmed" | "unconfirmed") ?? "confirmed",
        };
      }),
    };
  });

export const ensureDraftSchedule = protectedProcedure
  .input(z.object({ week_start: z.string().regex(/^\d{4}-\d{2}-\d{2}$/) }))
  .mutation(async ({ ctx, input }) => {
...
export const publishSchedule = protectedProcedure
...
export const unpublishSchedule = protectedProcedure
  .input(z.object({ schedule_id: z.string().uuid() }))
  .mutation(async ({ ctx, input }) => {
    await requireManager(ctx);
    const { error } = await ctx.supabase
...
// -------------------------------------------------------------------
// Shifts (with new fields + lock/double-booking checks)
// -------------------------------------------------------------------

export const createShift = protectedProcedure
  .input(CreateShiftInput)
  .mutation(async ({ ctx, input }) => {
    await requireManager(ctx);

    // Lock check
    await checkScheduleLock(ctx.supabase, input.schedule_id);

    // Double-booking check when a user is assigned
    if (input.user_id) {
      ...
      if (isDouble) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "User already has an overlapping shift in this time range",
        });
      }
    }
...
export const updateShift = protectedProcedure
  .input(UpdateShiftInput)
  .mutation(async ({ ctx, input }) => {
    await requireManager(ctx);

    // Look up the schedule_id for this shift so we can check the lock
    const { data: shiftRow } = await ctx.supabase
      .from("scheduling_shifts")
      .select("schedule_id")
      .eq("id", input.id)
      .maybeSingle();
    if (shiftRow) {
      await checkScheduleLock(ctx.supabase, shiftRow.schedule_id);
    }
    ...
    if (input.is_mod !== undefined) patch.is_mod = input.is_mod;
...
export const deleteShift = protectedProcedure
...
export const autoSuggest = protectedProcedure
  .input(
    z.object({
...
export const previewImport = protectedProcedure
  .input(
    z.object({
...
export const commitImport = protectedProcedure
  .input(
    z.object({
      shifts: z.array(
        z.object({
